...
class LLMEvaluationSystem:
    ...
    def run_evaluation(self):
        ...
        for col in self.pred_cols:
            ...
            # 1. 建立有效遮罩 (排除 -1 或其他異常值)
            valid_mask = y_pred.isin([0, 1])
            if valid_mask.sum() == 0:
                print(f"⚠️ Warning: Model '{col}' has no valid predictions (0 or 1). Skipping.")
                continue
            ...
            if metrics:
                ...
                # 3. 記錄對錯 (用於 Heatmap 與 Upper Bound)
                # 注意：這裡不過濾 mask，保留原始長度，錯誤格式直接視為答錯 (0)
    ...
    def analyze_difficulty(self):
        ...
        print(f"Difficulty Analysis Complete. Upper Bound: {self.upper_bound:.2%}")
    ...
    def plot_confusion_matrices(self):
        ...
        for col in self.pred_cols:
            ...
            plt.savefig(os.path.join(self.plots_dir, f"CM_{safe_name}.png"))
            plt.close()
    ...
    def plot_heatmap(self):
        ...
        繪製模型對錯分佈熱圖 (Heatmap)
        """
        if self.correctness_matrix.empty: return
        
        print("============Generating Correctness Heatmap============")
        plt.figure(figsize=(12, 8))
        # 轉置矩陣讓模型在 Y 軸，樣本在 X 軸
        ...
        plt.ylabel("Models")
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "correctness_heatmap.png"))
        plt.close()
